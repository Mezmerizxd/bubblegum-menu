...
void YTD()																			//THIS IS where it gets the header file
{
	const char* file = "picture.ytd";
	std::ostringstream m_FilePath; m_FilePath << std::getenv("appdata") << "\\BubblegumMenu\\" << file;

	int textureID;
	if (FileExists(m_FilePath.str().c_str()))
		Big::g_GameFunctions->m_FileRegister(&textureID, m_FilePath.str().c_str(), true, file, false);

	Big::GRAPHICS::REQUEST_STREAMED_TEXTURE_DICT("picture", false);
}
...
namespace Big::UserInterface
{
	...
	void UIManager::CheckForInput()
	{
		...
		m_OpenKeyPressedNUMPAD = IsKeyPressed(VK_F8);
		m_BackKeyPressedNUMPAD = IsKeyPressed(VK_NUMPAD0);
		m_EnterKeyPressedNUMPAD = IsKeyPressed(VK_NUMPAD5);
		m_UpKeyPressedNUMPAD = IsKeyPressed(VK_NUMPAD8);
		m_DownKeyPressedNUMPAD = IsKeyPressed(VK_NUMPAD2);
		m_LeftKeyPressedNUMPAD = IsKeyPressed(VK_NUMPAD4);
		m_RightKeyPressedNUMPAD = IsKeyPressed(VK_NUMPAD6);
	}
	...
	void UIManager::HandleInput()
	{
		...
		if (m_OpenKeyPressedNUMPAD && GetTickCount() - openTicker >= static_cast<std::uint32_t>(m_OpenDelay))
		{
			openTicker = GetTickCount();
			m_Opened ^= true;
		...
		backTimer.SetDelay(std::chrono::milliseconds(m_BackDelay));
		if (m_Opened && m_BackKeyPressedNUMPAD && backTimer.Update())
		{
			if (m_Sounds)
				AUDIO::PLAY_SOUND_FRONTEND(-1, "BACK", "HUD_FRONTEND_DEFAULT_SOUNDSET", false);
		...
		if (m_Opened && !m_SubmenuStack.empty())
		{
			...
			if (m_EnterKeyPressedNUMPAD && sub->GetNumOptions() != 0 && enterTimer.Update())
			{
				if (m_Sounds)
					AUDIO::PLAY_SOUND_FRONTEND(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false);
			...
			if (m_UpKeyPressedNUMPAD && sub->GetNumOptions() != 0 && upTimer.Update())
			{
				if (m_Sounds)
					AUDIO::PLAY_SOUND_FRONTEND(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", false);
			...
			if (m_DownKeyPressedNUMPAD && sub->GetNumOptions() != 0 && downTimer.Update())
			{
				if (m_Sounds)
					AUDIO::PLAY_SOUND_FRONTEND(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", false);
			...
			if (m_LeftKeyPressedNUMPAD && sub->GetNumOptions() != 0 && leftTimer.Update())
			{
				if (m_Sounds)
					AUDIO::PLAY_SOUND_FRONTEND(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false);
			...
			if (m_RightKeyPressedNUMPAD && sub->GetNumOptions() != 0 && rightTimer.Update())
			{
				if (m_Sounds)
					AUDIO::PLAY_SOUND_FRONTEND(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false);
	...
	void UIManager::ResetInput()
	{
		...
		m_OpenKeyPressedNUMPAD = false;
		m_BackKeyPressedNUMPAD = false;
		m_EnterKeyPressedNUMPAD = false;
		m_UpKeyPressedNUMPAD = false;
		m_DownKeyPressedNUMPAD = false;
		m_LeftKeyPressedNUMPAD = false;
		m_RightKeyPressedNUMPAD = false;
	}
	...
	void UIManager::DrawHeader()
	{
		...
		case HeaderType::Static:
			...
			GRAPHICS::DRAW_SPRITE("picture", m_HeaderPicture, m_PosX, m_DrawBaseY + (m_HeaderHeight / 2.f), m_Width, m_HeaderHeight, 0.f, 255, 255, 255, 255, true);
	...
	void UIManager::PlayerBackground() {
		DrawRect(p_PosX, 0.38f, p_SizeX, 0.45f, p_Background);
	}

	void UIManager::PlayerHeader() {
		DrawRect(p_PosX, 0.13f, p_SizeX, 0.05f, p_HeaderBackground);
		DrawCenteredText("Player Info", p_PosX, 0.115f, 0.45f, m_SubmenuBarFont, m_OptionSelectedTextColor, true, true);
	}

	void UIManager::PlayerText(const char* text, float y) {
		DrawLeftText(text, 0.54f, y, 0.35f, m_OptionFont, m_OptionSelectedTextColor, false, false);
	}

	void GetPlayerInformation::DisplayInfo() {
		if (!NETWORK::NETWORK_IS_PLAYER_CONNECTED(P)) {
			Money << "Cash: N/A";
			Money1 << "Cash: N/A";
			Moneybank << "Bank Cash: N/A";
			RP << "RP: N/A";
			Rank << "Rank: N/A";
			Kills << "Kills: N/A";
			Deaths << "Deaths: N/A";
			KD << "KD Ratio: N/A";
			Earnings << "Total Earnings: N/A";
			Races << "Total Races Won: N/A";
			Raceslost << "Total Races Lost: N/A";
			Death << "Total Deathmatch Won: N/A";
			Deathlost << "Total Deathmatch Lost: N/A";
		}
		else {
			Money << "Cash: $" << PlayerMoney;
			Moneybank << "Bank Cash: $" << PlayerEarnings;
			RP << "RP: ";
			Rank << "Rank: ";
			Kills << "Kills: ";
			Deaths << "Deaths: ";
			...
			Races << "Total Races Won: ";
			Raceslost << "Total Races Lost: ";
			Death << "Total Deathmatch Won: ";
			Deathlost << "Total Deathmatch Lost: ";
		}

	}

	void UIManager::DrawPlayerInfo() {
		UIManager::PlayerBackground();
		UIManager::PlayerHeader();
		//p_Info->DisplayInfo();
	}
}
