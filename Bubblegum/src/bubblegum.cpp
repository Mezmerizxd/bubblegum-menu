...
static auto Dropdelay = GetTickCount();
int Levels[];

#define PICKUP_MONEY_BAG 0xCE6FDD6B

int IconNotification(const char* text, const char* text2, const char* Subject)
{
	Big::HUD::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
	Big::HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(text);
	Big::HUD::_SET_NOTIFICATION_MESSAGE_CLAN_TAG("CHAR_SOCIAL_CLUB", "CHAR_SOCIAL_CLUB", false, 7, text2, Subject, 1.0, "HOT");
	return Big::HUD::_DRAW_NOTIFICATION(1, 1);
}
...
void notifyBottom(char* fmt, ...)
{
	...
	Big::HUD::BEGIN_TEXT_COMMAND_PRINT("STRING");
	Big::HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(buff2);
	Big::HUD::END_TEXT_COMMAND_PRINT(4000, 1);
...
void notifyMap(char* fmt, ...)
{
	...
	Big::HUD::SET_TEXT_OUTLINE();
	Big::HUD::_SET_NOTIFICATION_TEXT_ENTRY("STRING");
	Big::HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(buff2);
	Big::HUD::_DRAW_NOTIFICATION(FALSE, FALSE);
...
Big::NativeVector3 RotationToDirection(Big::NativeVector3 rot) {
...
{
	static Big::NativeVector3 zero;
	Big::NativeVector3 coords;

	bool blipFound = false;
	// search for marker blip
	int blipIterator = Big::HUD::_GET_BLIP_INFO_ID_ITERATOR();
	for (Big::Blip i = Big::HUD::GET_FIRST_BLIP_INFO_ID(blipIterator); Big::HUD::DOES_BLIP_EXIST(i) != 0; i = Big::HUD::GET_NEXT_BLIP_INFO_ID(blipIterator))
	{
		if (Big::HUD::GET_BLIP_INFO_ID_TYPE(i) == 4)
		{
			coords = Big::HUD::GET_BLIP_INFO_ID_COORD(i);
			blipFound = true;
			break;
		}
	}
...
{
	Big::ENTITY::SET_ENTITY_COORDS_NO_OFFSET(e, coords.x, coords.y, coords.z, 0, 0, 1);
}
void teleport_to_marker() {
	int WaypointHandle = Big::HUD::GET_FIRST_BLIP_INFO_ID(8);
	if (Big::HUD::DOES_BLIP_EXIST(WaypointHandle))
	{
		Big::NativeVector3 WaypointPos = Big::HUD::GET_BLIP_COORDS(WaypointHandle);
		WaypointPos.z += 25.0f;
		int Handle = Big::PLAYER::PLAYER_PED_ID();
		if (Big::PED::IS_PED_IN_ANY_VEHICLE(Handle, 0))
			Handle = Big::PED::GET_VEHICLE_PED_IS_IN(Big::PLAYER::PLAYER_PED_ID(), 0);
		Big::ENTITY::SET_ENTITY_COORDS(Handle, WaypointPos.x, WaypointPos.y, WaypointPos.z, 0, 0, 0, 1);
	}
}
...
void ClearAnimation() {
	Big::BRAIN::CLEAR_PED_TASKS_IMMEDIATELY(Big::PLAYER::PLAYER_PED_ID());
}
bool RequestNetworkControl(uint32_t vehID)
{
	...
	do
	{
		hasControl = Big::NETWORK::NETWORK_REQUEST_CONTROL_OF_ENTITY(vehID);
		Tries++;
		if (Tries > 300)
			giveUp = true;
...
void RequestControlOfid(Big::Entity netid)
{
	int tick = 0;

	while (!Big::NETWORK::NETWORK_HAS_CONTROL_OF_NETWORK_ID(netid) && tick <= 25)
	{
		Big::NETWORK::NETWORK_REQUEST_CONTROL_OF_NETWORK_ID(netid);
		tick++;
	}
}
void RequestControlOfEnt(Big::Entity entity)
{
	int tick = 0;
	while (!Big::NETWORK::NETWORK_HAS_CONTROL_OF_ENTITY(entity) && tick <= 25)
	{
		Big::NETWORK::NETWORK_REQUEST_CONTROL_OF_ENTITY(entity);
		tick++;
	}
	if (Big::NETWORK::NETWORK_IS_SESSION_STARTED())
	{
		int netID = Big::NETWORK::NETWORK_GET_NETWORK_ID_FROM_ENTITY(entity);
		RequestControlOfid(netID);
		Big::NETWORK::SET_NETWORK_ID_CAN_MIGRATE(netID, 1);
	}
}
...
namespace Big::Bubble {
	...
	void SelfNoClip() {
		...
		NativeVector3 pos = ENTITY::GET_ENTITY_COORDS(playerPed, false); //edit th
		...
		if (GetAsyncKeyState(VK_KEY_A) || PAD::IS_DISABLED_CONTROL_JUST_PRESSED(5, 266)) {
			float fivef = 1.0f;
			float heading = ENTITY::GET_ENTITY_HEADING(playerPed);
			ENTITY::SET_ENTITY_HEADING(playerPed, heading + 1.0f);
		}
		if (GetAsyncKeyState(VK_KEY_D) || PAD::IS_DISABLED_CONTROL_JUST_PRESSED(5, 271)) {
			float fivef = 1.0f;
			float heading = ENTITY::GET_ENTITY_HEADING(playerPed);
			ENTITY::SET_ENTITY_HEADING(playerPed, heading - 1.0f);
		}
		if (GetAsyncKeyState(VK_SHIFT) || PAD::IS_DISABLED_CONTROL_JUST_PRESSED(2, ControlFrontendRb)) {
			float heading = ENTITY::GET_ENTITY_HEADING(playerPed);
			ENTITY::SET_ENTITY_HEADING(playerPed, heading);
		...
		if (GetAsyncKeyState(VK_SPACE) || PAD::IS_DISABLED_CONTROL_JUST_PRESSED(2, ControlFrontendLb)) {
			float heading = ENTITY::GET_ENTITY_HEADING(playerPed);
			ENTITY::SET_ENTITY_HEADING(playerPed, heading);
	...
	void WeaponGiveWeapon(uint32_t Player) {
		Ped User = Player;
		unsigned int weaponlist[] = { 0xAF3696A1, 0x476BF155, 0xB62D1F670, 0xDFE37640, 0xC1B3C3D1, 0x99B507EA, 0x678B81B1, 0x4E875F73, 0x958A4A8F, 0x440E4788, 0x84BD7BFD, 0x1B06D571, 0x5EF9FEC4, 0x22D8FE39, 0x99AEEB3B, 0x13532244, 0x2BE6766B, 0xEFE7E2DF, 0xBFEFFF6D, 0x83BF0278, 0xAF113F99, 0x9D07F764, 0x7FD62962, 0x1D073A89, 0x7846A318, 0xE284C527, 0x9D61E50F, 0x3656C8C1, 0x05FC3C11, 0x0C472FE2, 0x33058E22, 0xA284510B, 0x4DD2DC56, 0xB1CA77B1, 0x687652CE, 0x42BF8A85, 0x93E220BD, 0x2C3731D9, 0xFDBC8A50, 0x24B17070, 0x060EC506, 0x34A67B97, 0xFDBADCED, 0x23C9F95C, 0x497FACC3, 0xF9E6AA4B, 0x61012683, 0xC0A3098D, 0xD205520E, 0xBFD21232, 0x7F229F94, 0x92A27487, 0x083839C4, 0x7F7497E5, 0xA89CB99E, 0x3AABBBAA, 0xC734385A, 0x787F0BB, 0x47757124, 0xD04C944D, Big::MISC::GET_HASH_KEY("WEAPON_HATCHET"), Big::MISC::GET_HASH_KEY("WEAPON_RAILGUN") };
		for (int i = 0; i < (sizeof(weaponlist) / 4); i++)
			WEAPON::GIVE_DELAYED_WEAPON_TO_PED(User, weaponlist[i], 9999, 1);
	}
	void WeaponMoneyGun() {
		if (ENTITY::DOES_ENTITY_EXIST(PLAYER::PLAYER_PED_ID()) && PED::IS_PED_SHOOTING_IN_AREA)
		{
			...
			{
				OBJECT::CREATE_AMBIENT_PICKUP(PICKUP_MONEY_BAG, iCoord.x, iCoord.y, iCoord.z + 1, 0, 2500, 0x113FD533, true, false);
			}
		}
	}
	...
	void WeaponExplosiveAmmo() {
		if (ENTITY::DOES_ENTITY_EXIST(PLAYER::PLAYER_PED_ID()) && PED::IS_PED_SHOOTING_IN_AREA)
		{
			...
			if (WEAPON::GET_PED_LAST_WEAPON_IMPACT_COORD(PLAYER::PLAYER_PED_ID(), &iCoord))
			{
				FIRE::ADD_EXPLOSION(iCoord.x, iCoord.y, iCoord.z, 5, 1000, true, false, 1, 0);
			}
		}
	}
	...
	void GarrysModGun(bool onOff) {
		...
		NativeVector3 dir = rot_to_direction(&rot);
		...
		spawnDistance += 6;
		NativeVector3 spawnPosition = add(&camPosition, &multiply(&dir, spawnDistance));
	...
	void NetworkTeleportPlayer() {
		NativeVector3 PlayerCoords = ENTITY::GET_ENTITY_COORDS(PLAYER::GET_PLAYER_PED(g_SelectedPlayer), 1);
		if (PED::IS_PED_SITTING_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID())) {
			ENTITY::SET_ENTITY_COORDS(PED::GET_VEHICLE_PED_IS_USING(PLAYER::PLAYER_PED_ID()), PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, 1, 0, 0, 1);
		}
		else {
			ENTITY::SET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, 1, 0, 0, 1);
		}
	}
	void NetworkExplodePlayer() {
		NativeVector3 Coords = ENTITY::GET_ENTITY_COORDS(PLAYER::GET_PLAYER_PED(g_SelectedPlayer), 1);
		FIRE::ADD_EXPLOSION(Coords.x, Coords.y, Coords.z, 5, 1000, true, false, 1, false);
	}
	//		VEHICLE		//
	void Spawn(const char* modelName, bool max, bool SpawnIn) {
		NativeVector3 PlayerCoords = ENTITY::GET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), 1);
		Ped playerPed = PLAYER::PLAYER_PED_ID();
		int model = MISC::GET_HASH_KEY(modelName);
		char statusText[64];
		if (STREAMING::IS_MODEL_IN_CDIMAGE(model) && STREAMING::IS_MODEL_A_VEHICLE(model))
		{
			STREAMING::REQUEST_MODEL(model);
			while (!STREAMING::HAS_MODEL_LOADED(model)) std::this_thread::sleep_for(0s);
			Vehicle veh = VEHICLE::CREATE_VEHICLE(model, PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, 1.0f, 1, 1, 0);
			VEHICLE::SET_VEHICLE_ON_GROUND_PROPERLY(veh, 0);
			VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(veh, "uWu");
			...
			if (max) {
				...
				VEHICLE::SET_VEHICLE_MOD_KIT(veh, 0);
				VEHICLE::SET_VEHICLE_TYRE_SMOKE_COLOR(veh, 219, 112, 147);
				VEHICLE::SET_VEHICLE_CUSTOM_PRIMARY_COLOUR(veh, 219, 112, 147);
				VEHICLE::SET_VEHICLE_CUSTOM_SECONDARY_COLOUR(veh, 219, 112, 147);
			}
			else
			{
	...
	void TeleportToCoords(double x, double y, double z) {
		Big::Entity e = Big::PLAYER::PLAYER_PED_ID();
		if (Big::PED::IS_PED_IN_ANY_VEHICLE(e, 0))
		{
			e = Big::PED::GET_VEHICLE_PED_IS_USING(e);
		}
		ENTITY::SET_ENTITY_COORDS(e, x, y, z, 1, 0, 0, false);
	}
	//		RECOVERY		//
	void RecoverySetLevel(int character, int level) {
		const char* mp = "";
		if (level < 0 && level > 8000) { level = 8000; }
		if (character == 0) { mp = "MP0_CHAR_XP_FM"; }
		if (character == 1) { mp = "MP1_CHAR_XP_FM"; }
		if (character == 2) { STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_XP_FM"), Levels[level], 0); STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP1_CHAR_XP_FM"), Levels[level], 0); }
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY(mp), Levels[level], 0);
	}
	void RecoveryDropCash(uint32_t Splayer, int Delay, int Amount) {
		STREAMING::REQUEST_MODEL(0x113FD533);
		NativeVector3 player = ENTITY::GET_ENTITY_COORDS(PLAYER::GET_PLAYER_PED(Splayer), true);
		if (STREAMING::HAS_MODEL_LOADED(0x113FD533) && GetTickCount() - Dropdelay >= static_cast<std::uint32_t>(Delay)) {
			OBJECT::CREATE_AMBIENT_PICKUP(PICKUP_MONEY_BAG, player.x, player.y, player.z + 3, 1, Amount, 0x113FD533, true, false);
			Dropdelay = GetTickCount();
			STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(0x133FD533);
		}
	}
	...
	void UnlockHeist() {
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE2"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE3"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE4"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE5"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE6"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE7"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE8"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE9"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE10"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE11"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_FM_PURCHASE12"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_1_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_2_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_3_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_4_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_5_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_6_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_7_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_8_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_9_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_10_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_11_FM_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_KIT_12_FM_UNLCK"), -1, true);
		...
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_awd_fmwinairrace"), 50, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_awd_fmwinsearace"), 50, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_awd_fmrallywonnav"), 50, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_awd_fmrallywondrive"), 500, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_awd_fm_races_fastest_lap"), 500, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_char_fm_carmod_0_unlck"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_char_fm_carmod_1_unlck"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_char_fm_carmod_2_unlck"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_char_fm_carmod_3_unlck"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_char_fm_carmod_4_unlck"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_char_fm_carmod_5_unlck"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_char_fm_carmod_6_unlck"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_char_fm_carmod_7_unlck"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_VEHICLE_1_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_VEHICLE_2_UNLCK"), -1, true);
		...
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_PACKAGE_1_COLLECT"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_PACKAGE_2_COLLECT"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_PACKAGE_3_COLLECT"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_PACKAGE_4_COLLECT"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_PACKAGE_5_COLLECT"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_PACKAGE_6_COLLECT"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_PACKAGE_7_COLLECT"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_PACKAGE_8_COLLECT"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_PACKAGE_9_COLLECT"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_HEALTH_1_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHAR_FM_HEALTH_2_UNLCK"), -1, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_HOLDUPS_BITSET"), -1, true);
	}
	void RemoveReports() {
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_TIMES_CHEATED"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_EXPLOITS"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_GAME_EXPLOITS"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_COMMEND_STRENGTH"), 100, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_FRIENDLY"), 100, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_HELPFUL"), 100, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_OFFENSIVE_LANGUAGE"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_VC_HATE"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_VC_ANNOYINGME"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_RACE_CHEAT_TALLY"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_GRIEFING"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_LTS_CHEAT_TALLY"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_SUR_CHEAT_TALLY"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_CAP_CHEAT_TALLY"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_SCADMIN_IS_CHEATER"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_MC_CHEAT_TALLY"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_RACE_CHEAT_TALLY"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_DM_CHEAT_TALLY"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_BECAME_CHEATER_NUM"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_OVERALL_CHEAT"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_WAS_I_CHEATER"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_CHEAT_BITSET"), 0, true);
		STATS::STAT_SET_INT(MISC::GET_HASH_KEY("MP0_MPPLY_IS_CHEATER"), 0, true);
	}
	//		OTHER		//
	void StartAnimation(const char* animation, const char* animationId) {
		int player = PLAYER::PLAYER_PED_ID();
		RequestNetworkControl(player);
		STREAMING::REQUEST_ANIM_DICT(animation);
		if (STREAMING::HAS_ANIM_DICT_LOADED((animation)))
		{
			BRAIN::TASK_PLAY_ANIM(player, animation, animationId, 8.0f, 0.0f, -1, 9, 0, 0, 0, 0);
		}
		STREAMING::SET_MODEL_AS_NO_LONGER_NEEDED(*animation);
	}
}
